...
public class LoginFunctionality extends BaseTest 
 {
	...
	public void ValidatingLoginFunctionalityTest()
	{
	   ...
	   catch(Exception e)
	   {
		...
		private void ValidateHomePage(int rowIndex) throws IOException
		{
	       ...
	       if(actual_HomePageText.contains(expected_HomePageText))
	       {
	    	   ...
	    	   Log.info("Successfully navigate to OrangeHRM Application homepage-pass");
	    	   
	    	   excelfile.SetCellValue(rowIndex, 7,"Successfully navigate to OrangeHRM Homepage-pass");
	       }
	       else
	       {
	    	   ...
	    	   OHRMScreenShot.takesScreenshot(driver, "failed to navigate Homepage");
	    	   excelfile.SetCellValue(rowIndex, 7,actual_HomePageText );
	       }
      ...
      private  void logout()
      {
         ...
         By homepageLogoutElement=By.linkText("logOutProperty");
         driver.findElement(homepageLogoutElement).click();
      }
